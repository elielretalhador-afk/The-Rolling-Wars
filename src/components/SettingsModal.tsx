...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const [editPhone, setEditPhone] = useState(settings.account.phone);
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {activeCategory === 'CONTA' && (
            <div className="space-y-4 animate-in fade-in">
              ...
              <div className="p-4 rounded-2xl bg-[#0a0a0a] border border-white/10 space-y-3">
                ...
                {isEditingProfile ? (
                  <div className="space-y-3 text-xs font-mono-stat">
                    <div>
                      <label className="text-slate-400 mb-1 block">Nome Completo</label>
                      <input 
                        type="text" 
                        value={editName}
                        onChange={(e) => setEditName(e.target.value)}
                        className="w-full bg-black border border-white/10 rounded-xl p-2.5 text-white focus:border-yellow-400 outline-none"
                      />
                    </div>
                    <div>
                      <label className="text-slate-400 mb-1 block">Nome de Usuário</label>
                      <input 
                        type="text" 
                        value={editNickname}
                    ...
                    <div>
                      <label className="text-slate-400 mb-1 block">Telefone</label>
                      <input 
                        type="tel" 
                        value={editPhone}
                    ...
                    <div className="flex items-center gap-2 pt-2">
                      <button
                        ...
                        onClick={() => {
                          ...
                          setEditPhone(settings.account.phone);
                          setIsEditingProfile(false);
                        }}
                        className="flex-1 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold text-slate-300 font-mono-stat uppercase tracking-wider transition-colors cursor-pointer"
                      >
                        Cancelar
                      </button>
                      <button
                        ...
                        onClick={() => {
                          if (onUpdateUser) {
                            onUpdateUser({ name: editName, nickname: editNickname });
                          }
                          updateSetting('account', 'phone', editPhone, 'Perfil atualizado com sucesso.');
                          setIsEditingProfile(false);
                        }}
                        className="flex-1 py-2.5 rounded-xl bg-yellow-400 hover:bg-yellow-300 text-black text-xs font-bold font-mono-stat uppercase tracking-wider transition-colors cursor-pointer"
                      >
                        Salvar
                      </button>
                    </div>
                  ...
                  <>
                    <div className="space-y-2 text-xs font-mono-stat">
                      ...
                      <div className="flex items-center justify-between p-2.5 rounded-xl bg-black/40 border border-white/5">
                        ...
                        <span className="text-white font-bold">{settings.account.phone || 'Não informado'}</span>
                      </div>
          ...
          {activeCategory === 'SOBRE' && (
            <div className="space-y-4 animate-in fade-in">
              <div className="p-6 rounded-2xl bg-gradient-to-b from-[#111822] to-[#080B0E] border border-yellow-500/30 text-center space-y-3 relative overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.8)]">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-24 bg-yellow-400/10 rounded-full blur-2xl pointer-events-none"></div>
                
                {/* LOGO NOVO THE ROLLING WARS */}
                <div className="relative w-28 h-28 mx-auto flex items-center justify-center">
                  <div className="absolute inset-0 bg-[#fce803] rounded-full blur-[25px] opacity-25 animate-pulse"></div>
                  <img
                    src="/logo-rw-dark.png"
                    alt="The Rolling Wars"
                    className="relative z-10 w-full h-full object-contain drop-shadow-[0_0_20px_rgba(252,232,3,0.45)]"
                    onError={(e) => { e.currentTarget.style.display = 'none'; }}
                  />
                  <div className="absolute inset-0 flex items-center justify-center border border-yellow-400/30 rounded-2xl -z-0">
                    <span className="text-yellow-400 font-black text-2xl tracking-widest opacity-40">RW</span>
                  </div>
                </div>

                <div>
                  <h3 className="text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-yellow-400 to-amber-500 font-display uppercase tracking-wider filter drop-shadow-[0_0_10px_rgba(252,232,3,0.3)]">
                    THE ROLLING WARS
                  </h3>
                  <p className="text-xs text-yellow-400 font-mono-stat font-bold mt-0.5">
                    Versão {settings.about.appVersion}
                  </p>
                  <p className="text-[10px] text-slate-400 font-mono-stat mt-1">
              ...
              <div className="p-4 rounded-2xl bg-[#0a0a0a] border border-white/10 space-y-2">
                ...
                {onOpenBetaDiagnostics && (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-black uppercase tracking-wider text-slate-300 font-display transition-colors cursor-pointer"
                  >
                    <div className="flex items-center gap-2">
                      <Settings className="w-4 h-4 text-slate-400" />
                      Painel de Diagnóstico Beta
                    </div>
