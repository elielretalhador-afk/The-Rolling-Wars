  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#000000] flex flex-col font-sans">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-[#081330] border-b border-[#000000]/30 shadow-md">
        <button onClick={goBack} className="p-2 -ml-2 text-neutral-300 hover:text-white rounded-full">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex items-center gap-1">
          <button 
            onClick={() => navigateTo('feed')} 
            className={`p-2 rounded-xl transition-colors ${activeTab === 'feed' ? 'text-yellow-400 bg-yellow-400/10' : 'text-neutral-300 hover:text-white hover:bg-white/5'}`}
          >
            <Newspaper className="w-5 h-5" />
          </button>
          <button 
            onClick={() => navigateTo('friends')} 
            className={`p-2 rounded-xl transition-colors ${activeTab === 'friends' ? 'text-yellow-400 bg-yellow-400/10' : 'text-neutral-300 hover:text-white hover:bg-white/5'}`}
          >
            <Users className="w-5 h-5" />
          </button>
          <button 
            onClick={() => navigateTo('search')}
            className={`p-2 rounded-xl transition-colors ${activeTab === 'search' ? 'text-yellow-400 bg-yellow-400/10' : 'text-neutral-300 hover:text-white hover:bg-white/5'}`}
          >
            <Search className="w-5 h-5" />
          </button>
          <button 
            onClick={() => navigateTo('chats')}
            className={`relative p-2 rounded-xl transition-colors ${activeTab === 'chats' || activeTab === 'chat_room' ? 'text-yellow-400 bg-yellow-400/10' : 'text-neutral-300 hover:text-white hover:bg-white/5'}`}
          >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/10">
                    <div className="flex gap-2">
                      <button className="text-neutral-400 hover:text-yellow-400"><Camera className="w-4 h-4" /></button>
                      <button className="text-neutral-400 hover:text-yellow-400"><Video className="w-4 h-4" /></button>
                    </div>
                    <button 
                  ...
                  <div key={`act-${act.id}-${i}`} className="bg-[#102a70]/30 border border-[#000000]/30 rounded-2xl p-4">
                    <div className="flex items-center gap-3 mb-3">
                      <img 
                        ...
                        className="w-10 h-10 rounded-full border border-neutral-800 object-cover cursor-pointer" 
                        onClick={() => {
                    ...
                    <div className="text-sm text-neutral-300 mb-3 whitespace-pre-wrap">{act.description}</div>
                    <div className="flex items-center gap-4 pt-3 border-t border-white/5">
                      <button className="flex items-center gap-1.5 text-neutral-400 hover:text-yellow-400 transition-colors">
                      ...
                      <button className="flex items-center gap-1.5 text-neutral-400 hover:text-yellow-400 transition-colors ml-auto">
        ...
        {activeTab === 'friends' && (
          <div className="p-4 max-w-lg mx-auto pb-24">
            ...
            {friendRequests.length > 0 && (
              <div className="mb-6">
                <h3 className="text-xs font-black text-neutral-400 uppercase tracking-wider mb-3">Solicitações de Amizade</h3>
                  ...
                  {friendRequests.map((req, i) => (
                    <div key={`req-${req.id}-${i}`} className="flex items-center justify-between p-3 bg-[#000000]/20 rounded-xl border border-yellow-400/30">
                      <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={() => openProfile(req)}>
                        <img src={req.avatar} className="w-10 h-10 rounded-full border border-neutral-800 bg-[#090d12]" />
                        <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => handleRejectFriend(req)} className="p-2 bg-slate-800 text-neutral-400 hover:text-white rounded-lg">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    </div>
                  ...
                  <div key={`friend-${friend.id}-${i}`} className="flex items-center justify-between p-3 bg-[#000000]/10 rounded-xl border border-[#000000]/30">
                    <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={() => openProfile(friend)}>
                      <div className="relative">
                        <img src={friend.avatar} className="w-10 h-10 rounded-full border border-neutral-800 bg-[#090d12]" />
                    ...
                    <button onClick={() => openChat(friend)} className="p-2 text-neutral-400 hover:text-yellow-400 bg-white/5 rounded-lg">
          ...
          <div className="p-4 max-w-lg mx-auto">
            ...
            <div className="flex gap-2 mb-4">
              {['ALL', 'PLAYERS', 'POSTS'].map(tab => (
                <button 
                  ...
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider ${searchTab === tab ? 'bg-yellow-400 text-black' : 'bg-white/5 text-neutral-400'}`}
                >
            ...
            {isSearching ? (
              ...
              <div className="text-center py-12">
                ...
                <h4 className="text-sm font-bold text-neutral-300 uppercase">Nenhum resultado encontrado</h4>
              ...
              <div className="space-y-6">
                {(searchTab === 'ALL' || searchTab === 'PLAYERS') && searchResultsPlayers.length > 0 && (
                  <div>
                    <h3 className="text-xs font-black text-neutral-400 uppercase tracking-wider mb-3">Jogadores</h3>
                    <div className="space-y-2">
                      {searchResultsPlayers.map((p, i) => (
                        <div key={`search-player-${p.id}-${i}`} className="flex items-center gap-3 p-3 bg-[#000000]/10 rounded-xl border border-[#000000]/30 cursor-pointer hover:border-yellow-400/50" onClick={() => openProfile(p)}>
                          <img src={p.avatar} className="w-10 h-10 rounded-full border border-neutral-800 bg-[#090d12]" />
                          <div>
                ...
                {(searchTab === 'ALL' || searchTab === 'POSTS') && searchResultsPosts.length > 0 && (
                  <div>
                    <h3 className="text-xs font-black text-neutral-400 uppercase tracking-wider mb-3">Publicações</h3>
                      ...
                      {searchResultsPosts.map((act, i) => (
                        <div key={`act-${act.id}-${i}`} className="bg-[#000000]/10 border border-[#000000]/30 rounded-xl p-3">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-xs font-bold text-neutral-300">{act.playerNickname}</span>
                          </div>
                          <p className="text-sm text-neutral-400 line-clamp-2">{act.description}</p>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="space-y-2">
                {chats.map((chat, i) => (
                  <div key={`chat-${chat.id}-${i}`} className="flex items-center gap-3 p-3 bg-[#000000]/10 rounded-xl border border-[#000000]/30 cursor-pointer hover:border-yellow-400/50" onClick={() => openChat(chat.participant)}>
                    <div className="relative">
                      <img src={chat.participant.avatar} className="w-12 h-12 rounded-full border border-neutral-800 bg-[#090d12]" />
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-neutral-400 truncate">{chat.lastMessage?.text || 'Inicie uma conversa'}</div>
                    </div>
                    {chat.unread > 0 && (
        ...
        {activeTab === 'profile' && selectedPlayer && (
          <div className="p-0">
            ...
            <div className="relative">
              ...
              <div className="absolute top-16 inset-x-0 flex flex-col items-center">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="px-2 py-0.5 rounded bg-white/10 text-xs font-bold text-neutral-300">Nvl {selectedPlayer.level || 1}</span>
                  <span className="px-2 py-0.5 rounded bg-white/10 text-xs font-bold text-neutral-300">{selectedPlayer.xp || 0} XP</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-32 px-6 flex flex-col gap-3">
              <div className="flex justify-center gap-3">
                {selectedPlayer.id === currentUser.authId ? (
                  ...
                  <button onClick={() => handleRemoveFriend(selectedPlayer)} className="flex-1 py-2.5 rounded-xl bg-slate-800 border border-neutral-800 text-neutral-300 hover:text-white font-black uppercase tracking-wider text-xs flex justify-center items-center gap-2 transition-colors">
            ...
            <div className="mt-8 px-4 max-w-lg mx-auto">
              <h3 className="text-xs font-black text-neutral-400 uppercase tracking-wider mb-4 border-b border-white/10 pb-2">Estatísticas</h3>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#000000]/10 border border-[#000000]/30 rounded-xl p-3 text-center">
                  ...
                  <div className="text-[10px] text-neutral-400 uppercase tracking-wider font-bold">Zonas</div>
                </div>
                <div className="bg-[#000000]/10 border border-[#000000]/30 rounded-xl p-3 text-center">
                  <div className="text-2xl font-black text-white font-mono-stat">{selectedPlayer.totalKm || 0}</div>
                  <div className="text-[10px] text-neutral-400 uppercase tracking-wider font-bold">KM Rodados</div>
                </div>
              </div>
            </div>
          </div>
